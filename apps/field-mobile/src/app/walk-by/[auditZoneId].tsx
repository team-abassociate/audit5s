  ...
  return (
    <Card style={styles.photoCard}>
      ...
      <View style={styles.photoHeader}>
        <Text style={[styles.classification, { color: classificationColor(photo.classification) }]}>
  ...
  return (
    <View style={styles.choices}>
      {CLASSIFICATIONS.map((classification) => (
        ...
        >
          <Text style={{ color: classificationColor(classification), fontWeight: '600' }}>
...
function classificationColor(value: string): string {
  return value === 'GOOD' ? theme.color.success : value === 'NONCONFORMITY' ? theme.color.danger : theme.color.textMuted;
}

const styles = StyleSheet.create({
  centered: { alignItems: 'center', justifyContent: 'center' },
  content: { gap: theme.space.sm, paddingBottom: theme.space.xl },
  heading: { fontSize: theme.font.lg, fontWeight: '700', color: theme.color.text },
  body: { color: theme.color.text, fontSize: theme.font.base },
  captureCard: { gap: theme.space.sm },
  choices: { flexDirection: 'row', gap: theme.space.xs },
  choice: { flex: 1, alignItems: 'center', borderWidth: 1, borderColor: theme.color.border, borderRadius: theme.radius.sm, padding: theme.space.sm },
  choiceSelected: { borderColor: theme.color.brand, backgroundColor: '#FFF7F3' },
  photoCard: { gap: theme.space.sm },
  photo: { width: '100%', aspectRatio: 4 / 3, borderRadius: theme.radius.sm, backgroundColor: theme.color.background },
  ...
  classification: { fontWeight: '700' },
  flagged: { color: theme.color.accent, fontWeight: '700', fontSize: theme.font.sm },
  fieldLabel: { fontSize: theme.font.sm, fontWeight: '600', color: theme.color.textMuted, marginBottom: theme.space.xs },
  input: { minHeight: 72, borderWidth: 1, borderColor: theme.color.border, borderRadius: theme.radius.sm, padding: theme.space.md, color: theme.color.text, textAlignVertical: 'top' },
});
