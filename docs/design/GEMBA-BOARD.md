...
|---|---|---|
| Good | ≥ 80 | `--ok` / `--ok-band` |
| Watch | 60 – 79.9 | `--warn` / `--warn-band` |
| Action | < 60 | `--crit` / `--crit-band` |
| Target line | 85 | `--crit-band`, dashed, on charts only |
| Not applicable | — | `.gb-na` hatch, label `N/A` |
...
- Scores are `numeric(6,3)`. Show **one decimal** in tiles and charts, **three** in ledgers, registers and anything that stands in for a record. Never round in a way that changes a band.
